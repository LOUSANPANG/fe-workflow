...
```bash
# 1、生成新的SSH key；注意生成 id_ras 时最好前缀加上标识 xx_id_rsa；
ssh-keygen -t rsa -C "xx@xx.com"
# 2、将新的SSH key添加到ssh-agent
ssh-add ~/.ssh/xx_id_rsa
# 3、复制公钥内容添加到自己的Git账户
cat xx_id_rsa.pub | pbcopy
# 4、config添加多个ssh-key
# 在 ~/.ssh 目录下新建 config 文件，添加如下内容
Host git@github.com
  HostName https://github.com
  User git
  IdentityFile ~/.ssh/xx_id_rsa
Host git@company.gitlab.com
  HostName https://company.gitlab.com
  User git
  IdentityFile ~/.ssh/xx_id_rsa
```
